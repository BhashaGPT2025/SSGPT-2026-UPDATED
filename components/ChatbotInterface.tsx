...
const ChatbotInterface: React.FC<{ onGenerate: (formData: FormData) => void }> = ({ onGenerate }) => {
  ...
  const initChat = useCallback(async () => {
    ...
    try {
      if (!process.env.API_KEY) throw new Error("API Key is missing. Please check your .env.local file.");
      ...
      if (!importedFilesRaw) {
        // Initial greeting
        const newBotMessage: Message = { id: `bot-${Date.now()}`, sender: 'bot', text: "Hello! I'm SSGPT. How can I assist you with your exam preparation today?" };
        setMessages([newBotMessage]);
      }
    } catch (error: any) { 
        console.error("Failed to initialize chatbot:", error); 
        setMessages([{ id: 'bot-error', sender: 'bot', text: `System Error: ${error.message || "Failed to connect to AI service."}` }]);
