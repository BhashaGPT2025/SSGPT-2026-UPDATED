...
// Fix: Removed `(chunk as any)` cast and accessed `.text` directly on the stream chunk.
        for await (const chunk of responseStream) {
            if (chunk.text) {
                setMessages(prev => prev.map(msg => msg.id === newBotMessage.id ? {...msg, text: msg.text + chunk.text} : msg)); 
            }
        }
      }
...
// Fix: Removed `(chunk as any)` casts to directly access properties on the stream chunk.
      for await (const chunk of responseStream) {
      ...
      if (functionCalls.length > 0 && functionCalls[0].name === 'generatePaper' && functionCalls[0].args) { 
          handleTriggerGeneration(functionCalls[0].args); 
          return; 
      }
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      } catch (error) { console.error("Error processing file:", error); alert("Sorry, there was an an error processing that file."); }
    }
...
// Fix: Use the correct model for live audio sessions as per the latest guidelines.
